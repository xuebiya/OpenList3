...
func Init(e *gin.Engine) {
	...
	mediaLog := middlewares.MediaAccessLog()
	g.GET("/d/*path", middlewares.PathParse, signCheck, downloadLimiter, mediaLog, handles.Down)
	g.GET("/p/*path", middlewares.PathParse, signCheck, downloadLimiter, mediaLog, handles.Proxy)
	g.HEAD("/d/*path", middlewares.PathParse, signCheck, mediaLog, handles.Down)
	g.HEAD("/p/*path", middlewares.PathParse, signCheck, mediaLog, handles.Proxy)
	...
	g.GET("/sd/:sid", middlewares.EmptyPathParse, middlewares.SharingIdParse, downloadLimiter, mediaLog, handles.SharingDown)
	g.GET("/sd/:sid/*path", middlewares.PathParse, middlewares.SharingIdParse, downloadLimiter, mediaLog, handles.SharingDown)
	g.HEAD("/sd/:sid", middlewares.EmptyPathParse, middlewares.SharingIdParse, mediaLog, handles.SharingDown)
	g.HEAD("/sd/:sid/*path", middlewares.PathParse, middlewares.SharingIdParse, mediaLog, handles.SharingDown)
